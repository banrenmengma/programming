...
double g_Trail[N_CITY_COUNT][N_CITY_COUNT]; //两两城市间信息素，就是环境信息素
...
int CAnt::ChooseNextCity()
{
	...
	for (int i=0;i<N_CITY_COUNT;i++)
	{
		...
		{
			prob[i]=pow(g_Trail[m_nCurCityNo][i],ALPHA)*pow(1.0/g_Distance[m_nCurCityNo][i],BETA); //该城市和当前城市间的信息素
	...
	//===================================================
	//如果城市间的信息素非常小，那么由于浮点运算的误差原因，上面的概率总和可能为0。会出现没有城市被选择出来的情况。出现这种情况，就把第一个没去过的城市作为返回结果
...
void CTsp::UpdateTrail()
{
	...
	for (int i=0;i<N_CITY_COUNT;i++)
	{
		for (int j=0;j<N_CITY_COUNT;j++)
		{
			g_Trail[i][j]=g_Trail[i][j]*ROU+dbTempAry[i][j]; //最新的环境信息素=留存的信息素+新留下的信息素
		}
	}
}
